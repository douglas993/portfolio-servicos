    ...
    getTodosIndex(divPortfolios) {
        ...
        promise.then(response => {
            ...
            for (const servico of response.dados) {
                dados += `<div class ="card" text-white bg-primary><div class="card-header"><h5 class="card-title">${servico.descricao}</h5></div><div class="card-body"><p class="card-text">${servico.detalhes}</p></div></div>`;
            }
